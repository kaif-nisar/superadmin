...
//     if (role === "admin") {
//         console.log("User is an admin, checking layer...");
//       // Try to get adminLayer from localStorage or API response
//       const adminLayer = data.user.adminLayer || localStorage.getItem("adminLayer") || "1";
...
function initializeMenu(userRole) {
  ...
  let role = userRole || "admin1"; // Default to admin1 if undefined
...
function debugAdminLayer() {
  ...
  const currentLayer = localStorage.getItem("adminLayer") || "1";
  layerSelector.value = currentLayer;
  console.log("Current Layer:", currentLayer);
  console.log("Role:", layerSelector.value);
  // Add event listener to change layer
  layerSelector.addEventListener("change", (e) => {
    const newLayer = e.target.value;
    localStorage.setItem("adminLayer", newLayer);
    
    // Update the UI immediately
    const roleWithLayer = `admin${newLayer}`;
