   ...
   async function  getFranchiseeById() {
    ...
    const _id = Params.get('testName');
    console.log('Extracted ID:', _id); // Debug line
    if (_id) {
...
async function updateFranchisee() {
    const Params = new URLSearchParams(window.location.search);
    const _id = Params.get('testName'); // Get the ID again for the update
    const status = document.getElementById('status').value === 'active'; // Convert to boolean
    console.log('kdsjfoidshfiu')
    const data = {
